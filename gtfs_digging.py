...
def create_gpx_files_from_csv(csv_path: pathlib.Path, output_dir: pathlib.Path):
    ...
    for index, row in tqdm(df.iterrows(), desc="Creating GPX traces", total=len(df)):
        trip_id = row['trip_id']
        stop_coordinates = eval(row['stop_coordinate_array'])  # Convert string representation of list to actual list
...
print(f"Removed {i_len - len(stop_times):,} rows for being outside of the time window: {departure_time.time()} - {trip_end_time.time()}")

# THings to investigate
top_check = stop_times.loc[stop_times["trip_id"] == "VJd52ffba9e123b1a8d51fd4e3844e51e0d506f99f"].copy()
top_check_pls_be_the_same = stop_times.loc[stop_times["trip_id"] == "VJf470303a637ffc221bcc1c814e0e788f2c883b44"].copy()
...
assure_st_grp_test = stop_times.loc[stop_times["trip_id"] == "VJffffe5f8fd900a3e8e9e64e4691e556ddaeed637"].copy()
...
print(f"With {len(trip_stop_times):,} rows and {len(trip_stop_times['trip_id'].unique()):,} unique trip ids")

# Then apply groupby on route_id trip_id & idxmax on stop_sequence!
print(f"commencing groupby {ct()}")
trip_stop_times_grped = trip_stop_times.loc[trip_stop_times.groupby(["route_id", "trip_id"])["stop_sequence"].idxmax()].copy()
print(f"completed {ct()}")
print(f"leaving {len(trip_stop_times_grped):,} rows and {trip_stop_times_grped['trip_id'].nunique():,} unique trip ids")


i_len = len(trip_stop_times)
print(f"Rmoving trips with a stop departure time before departure_time {departure_time}")
...
for trip_id, trip_data in tqdm(grouped, desc="building coordinate arrays", total=len(grouped)):
    ...
    stop_times = [pd.to_datetime(t, format="%H:%M:%S").strftime("%H:%M:%S") for t in trip_data["departure_time"]]
...
df_trip_coordinates = pd.DataFrame(list(trip_coordinates.items()), columns=["trip_id", "stop_coordinate_array"])
